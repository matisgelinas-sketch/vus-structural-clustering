...
DIST_THRESHOLD_A = 6.0  # confirmed with user: tightened from 8-10A starting point
...
# chain-adjacent cases, and already-fixed the nearest-neighbor masking bug
...
# rank by whichever atom type gives the more extreme (larger) seq/3D ratio
...
summary.to_csv(OUT / "TP53_phase1_flagged_candidates.csv", index=False)
print(f"Saved summary table: {OUT / 'TP53_phase1_flagged_candidates.csv'} ({len(summary)} rows)")

# Also save the full VUS table (not just flagged) with low-confidence + domain
# annotations, per "don't silently drop anything" instruction.
vus_full_cols = ['variation', 'position', 'wt_aa', 'mt_aa', 'domain', 'plddt',
                  'low_confidence', 'nearest_pathogenic_pos', 'dist3d_A', 'seqdist_nearest3d',
                  'nearest_pathogenic_pos_cb', 'dist3d_cb_A', 'seqdist_cb',
                  'flagged_candidate', 'confirmed_by_both_atoms', 'condition']
vus[vus_full_cols].sort_values('dist3d_A').to_csv(OUT / "TP53_all_VUS_annotated.csv", index=False)
print(f"Saved full annotated VUS table: {OUT / 'TP53_all_VUS_annotated.csv'} ({len(vus)} rows)")
...
pdb_text = (BASE / "AF-P04637-F1-model_v6.pdb").read_text()
...
<div style="font-family: sans-serif; padding: 12px; max-width: 1000px;">
  <h3>TP53 &mdash; structural clustering candidates (hypothesis-generation only)</h3>
...
"""
(OUT / "TP53_structure_viz.html").write_text(legend + html)
print(f"Saved visualization: {OUT / 'TP53_structure_viz.html'}")
